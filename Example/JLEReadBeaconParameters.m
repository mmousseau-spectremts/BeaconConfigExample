...
- (void) ReadProximityUUID
{
    if (self.mSelectBeaconDevice.isConnected) {
        [self.mSelectBeaconDevice readBeaconProximityUUIDWithCompletion:^(NSString* value, NSError* error){
            ...
            NSLog(@"Reading Proximity ID:");
            NSLog(self.mUUID.text);
        }];
    }
}
...
{
    if (self.mSelectBeaconDevice.isConnected) {
        [self.mSelectBeaconDevice writeBeaconProximityUUID:@"C205DA90-DA03-470F-ACE1-16FBD632EDB1" withCompletion:^(BOOL value, NSError* error){
            
            if (value) {
                NSLog(@"Beacon Proximity UUID Config Success..");
                NSLog(self.mSelectBeaconDevice.macAddress );
                NSLog([self.mSelectBeaconDevice.peripheral.identifier UUIDString] );
//               NSLog(self.mSelectBeaconDevice.proximityUUID );
           }
...
{
    if (self.mSelectBeaconDevice.isConnected) {
        [self.mSelectBeaconDevice writeBeaconMajor:320 withCompletion:^(BOOL value, NSError* error){
...
{
    if (self.mSelectBeaconDevice.isConnected) {
        [self.mSelectBeaconDevice writeBeaconMinor:42 withCompletion:^(BOOL value, NSError* error){
...
{
    if (self.mSelectBeaconDevice.isConnected) {
        [self.mSelectBeaconDevice writeBeaconDeviceName:@"SpectreMTS" withCompletion:^(BOOL value, NSError* error){
            
            if (value) {
                NSLog(@"Beacon Device Name Config Success..SpectreMTS");
            }
